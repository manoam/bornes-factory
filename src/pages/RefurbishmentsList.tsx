...
const STATUS_KPIS: { status: Status; label: string; Icon: typeof Recycle; color: string }[] = [
  { status: 'DRAFT', label: 'Brouillons', Icon: Recycle, color: 'text-slate-600' },
...
export default function RefurbishmentsList() {
  const { user } = useAuth();
  ...
  const listQ = useQuery({
    queryKey: ['refurbishments', queryString],
    queryFn: async () => {
      const res = await api.get<ListResponse>(`/refurbishments?${queryString}`);
      return res.data;
    },
  });
  ...
  const createM = useMutation({
    ...
    onSuccess: (data) => {
      qc.invalidateQueries({ queryKey: ['refurbishments'] });
      setCreateOpen(false);
      window.location.href = `/refurbishments/${data.id}`;
    },
  });
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">Reconditionnements</h1>
          <p className="text-[13px] text-[--k-muted]">
            Bornes revenues à l'atelier pour révision. On retire, on remplace, on
            teste puis la borne repart en location.
          </p>
        </div>
        <button
          type="button"
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-[--k-surface] rounded-xl w-full max-w-md shadow-xl">
        <div className="px-4 py-3 border-b border-[--k-border] flex items-center justify-between">
          <h2 className="font-semibold">Nouveau reconditionnement</h2>
        ...
        <div className="p-4 space-y-3">
          <div>
            ...
            <p className="text-[11px] text-[--k-muted] mt-1">
              Le numéro ne change pas — on garde la traçabilité complète de la
              borne.
            </p>
          </div>
          <div>
            <label className="block text-[12px] font-medium text-[--k-muted] mb-1">
              Motif du reconditionnement (optionnel)
            </label>
            <textarea
              className="input-field py-2 min-h-[70px]"
              placeholder="ex : révision annuelle, retour location, mise à niveau…"
              value={reason}
