...
export default function RepairOrdersList() {
  ...
  const listQ = useQuery({
    queryKey: ['repair-orders', queryString],
    queryFn: async () => {
      const res = await api.get<ListResponse>(`/repair-orders?${queryString}`);
      return res.data;
    },
  });

  const createM = useMutation({
    mutationFn: async (payload: { borneInternalNumber: string; diagnosis?: string }) => {
    ...
    onSuccess: (data) => {
      ...
      setCreateOpen(false);
      window.location.href = `/repair-orders/${data.id}`;
    },
  });
  ...
  return (
    <div className="space-y-4">
      <header className="flex items-start justify-between gap-3">
        <div>
          <h1 className="text-xl font-semibold">Ordres de réparation</h1>
          <p className="text-[13px] text-[--k-muted]">
            Bornes existantes en cours de réparation. Une borne = un ordre.
          </p>
        </div>
        <button
          type="button"
      ...
      <section className="rounded-xl border border-[--k-border] bg-[--k-surface] p-3">
        <div className="flex flex-wrap items-center gap-2">
          <div className="relative flex-1 min-w-[200px]">
            ...
            <input
              type="text"
              placeholder="N° borne ou diagnostic…"
              value={search}
              onChange={(e) => {
                setOffset(0);
                    ...
                    <tr key={r.id}>
                      <td className="px-4 py-2">
                        <BorneCell
                          ...
                          chantierLink={`/repair-orders/${r.id}`}
                      ...
                      <td className="px-4 py-2 max-w-[300px]">
                        <div className="truncate text-[--k-text]">
                          {r.diagnosis || (
                            <span className="italic text-[--k-muted]">—</span>
                          )}
                        </div>
                      </td>
...
function CreateModal({
  ...
  onClose: () => void;
  onSubmit: (payload: { borneInternalNumber: string; diagnosis?: string }) => void;
  submitting: boolean;
  error: string | null;
}) {
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
      <div className="bg-[--k-surface] rounded-xl w-full max-w-md shadow-xl">
        <div className="px-4 py-3 border-b border-[--k-border] flex items-center justify-between">
          <h2 className="font-semibold">Nouvel ordre de réparation</h2>
        ...
        <div className="p-4 space-y-3">
          <div>
            ...
            <p className="text-[11px] text-[--k-muted] mt-1">
              Factory cherche d'abord dans les bornes fabriquées ici, sinon
              dans l'app Bornes.
            </p>
          </div>
          <div>
            ...
            <textarea
              ...
              placeholder="ex : imprimante ne répond plus, écran fissuré…"
              value={diagnosis}
